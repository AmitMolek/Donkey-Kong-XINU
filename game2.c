...
// the entire display in a fucking char array
char display[2001];

// Queue that saves the input from the player
...
void displayer(void) {
	while (1) {
		...
		receive();
		//sleept(18);
		// prints the whole display to the console
		printf(display);
	} //while
} // prntr
...
void updateter()
{
	...
	while (1) {
		...
		// run for all the chars in the input Queue (ch_arr)
		while (front != -1)
		{
			ch = ch_arr[front];
			if (front != rear)
